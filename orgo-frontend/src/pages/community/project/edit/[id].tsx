...
import { useUpdateProject } from '../../../../data/hooks/mutations/useMarketplace'
import { useGetProjectById } from '../../../../data/hooks/query/useMarketplaceQuery'
import { useRouter } from 'next/router'
import { ProjectRequirement } from '../new'
...
function EditProjectForm() {
  ...
  const getProjectRequirementField = (
    ...
    if (fieldData.type === 'TASK') {
      return (
        <Stack direction="row" mb="2" key={fieldData.title || fieldData.taskId}>
          <Select
  ...
  useEffect(() => {
    if (getProjectById.data) {
      console.log(getProjectById.data)
      formik.setFieldValue('name', getProjectById.data?.name)
      formik.setFieldValue('narrative', getProjectById.data?.description)
      formik.setFieldValue('category', getProjectById.data?.category[0])
      formik.setFieldValue(
        'budgetNarrative',
        getProjectById.data?.budgetNarrative
      )
      formik.setFieldValue('totalBudget', getProjectById.data?.totalBudget)
      formik.setFieldValue('status', getProjectById.data?.status)
      ...
      setPreviewLink(getProjectById.data?.image)
    }
  }, [getProjectById.data])
      ...
      >
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          >
            ...
            {files.length > 0 ? (
              <Alert mb="3">{files.length} File(s) selected</Alert>
            ) : null}
            {!files.length && previewLink ? (
              <Image mb="3" src={previewLink} alt={getProjectById.data?.name} />
            ) : null}
          ...
          <Button
            type="submit"
            isLoading={uploadFile.isLoading || updateProject.isLoading}
